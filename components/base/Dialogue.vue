<template>
  <div class="dialogue">
    <div class="dialogue__overlay" v-if="show" />
    <div
      class="dialogue__container gap-4 flex flex-col w-11/12 md:w-8/12 lg:w-6/12 xl:w-4/12 2xl:w-3/12 border rounded min-h-[12rem] relative"
      :class="
        classes['type']['dialogue'] +
        ' ' +
        classes['shape'] +
        ' ' +
        classes['size']
    ...
    >
      <div class="dialogue__header">
        <h3
          class="dialogue__title text-xl font-bold"
          :class="classes['type']['header']"
        >
          {{ title }}
        </h3>
      </div>
      <div class="dialogue__body" :class="classes['type']['body']">
        <p class="dialogue__message">{{ message }}</p>
      </div>
      <div
        class="dialogue__footer self-end flex gap-4 absolute bottom-4 right-4"
      >
        <BaseButton :type="type" :shape="shape" outlined @click="cancel"
          >Cancelar</BaseButton
        >
        <BaseButton :type="type" :shape="shape" @click="confirm"
...
let types = {
  primary: {
    dialogue: "bg-muted-500 text-primary-500 border-primary-500",
    header: "text-primary-500",
    body: "text-primary-400",
  },
  success: {
    dialogue: "bg-muted-500 border-success-500",
    header: "text-success-500",
    body: "text-success-400",
  },
  error: {
    dialogue: "bg-muted-500 border-error-500",
    header: "text-error-500",
    body: "text-error-400",
  },
  warning: {
    dialogue: "bg-muted-500 border-warning-500",
    header: "text-warning-500",
    body: "text-warning-400",
  },
  info: {
    dialogue: "bg-muted-500 border-info-500",
